...
if ( ! function_exists( 'uswds_posted_on' ) ) :
/**
...
function uswds_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'uswds' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'uswds' ),
...
function uswds_entry_footer() {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( esc_html__( 'Leave a comment', 'uswds' ), esc_html__( '1 Comment', 'uswds' ), esc_html__( '% Comments', 'uswds' ) );
		echo '</span>';
	}
